...
function SplitFlapChar({ char, index, animationKey, skipEntrance, speed, playClick }: SplitFlapCharProps) {
  ...
  const bgColor = isSettled ? "hsl(0, 0%, 0%)" : "color-mix(in srgb, var(--accent) 20%, transparent)"
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 top-0 bottom-1/2 flex items-end justify-center overflow-hidden">
        <span
          className={`block translate-y-[0.52em] leading-none transition-colors duration-150 ${["W", "M"].includes(currentChar) ? "scale-x-[0.75]" : ""}`}
